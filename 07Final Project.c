...
In fact, the radius is calculated from other data that is measured, namely the star's brightness
(also called luminosity) and its surface temperature.
More precisely, the surface temperature cannot be directly measured either but is itself computed,
but let's not go there here.

You are given a file, stardata.txt, with star data: Each line of the file starts with the name of a star
(a string), followed by the star's temperature (in Kelvin), followed by the star's luminosity
(measured in terms of the Sun's luminosity).
...
You next task is to write a function that computes the radius of each star in this array
(in terms of the Sun's radius) and stores this radius in the appropriate location in the array of structures.
The formula to compute a star's radius (in terms of the Sun's radius) is R = (TS / T)² * sqrt(L),
where TS=3500 is the temperature (in Kelvin) of the sun, T is the temperature of the star and L is the luminosity
of the star in terms of the Sun's luminosity. 

Your final task is to write a function which classifies each star in the array of structures as from the Main Sequence
(store an 'M' in the appropriate location in the structure), a Giant ('G'), a Supergiant ('S'), or a White Dwarf ('W').
If a star falls into neither of these categories store an 'N' in the appropriate location.
...
You are given the structure declaration along with a main() function and a printStars() function.
Do not modify any of these.
Your job is to write the three functions which are called within the main() function.
